...
table <- read.csv("all_mag_abundance_mat.tsv", header=TRUE, sep='\t', row.names="samples")
...
metadata$Target.Type <- as.factor(metadata$Target.Type)
...
plot_sub <- subset(for_plot, Target!='ARO')


c <- ggplot(for_plot, aes(x=MDS1, y=MDS2, color=Type, shape=Target)) +
  geom_point(size=2.5) +
  scale_color_manual(values=c("#ef476f", "#ffd166", "#06d6a0", "#073b4c")) +
  scale_shape_manual(values=c(16,17,4)) +
  theme_minimal(base_size=16) +
  stat_ellipse(data=plot_sub, aes(x=MDS1, y=MDS2, group = interaction(Type, Target), color=Type, fill=Type), 
               geom = "polygon",
               alpha=0.1) +
  xlab("NMDS1") +
  ylab("NMDS2")
...
adonis2(bray_dist~metadata$Target.Type, 
        strata=metadata$patient_id,
        by="terms")
